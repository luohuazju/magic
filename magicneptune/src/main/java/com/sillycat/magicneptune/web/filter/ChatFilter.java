...
package com.sillycat.magicneptune.web.filter;
...
public class ChatFilter extends AjaxFilter {
	...
	private void doLeaveChat(Map room, String name, HttpServletRequest request,
		...
		// response.objectResponse("left",
		// "<left from=\""+member.getName()+"\"/>");
		sendMembers(room, response);
	}
	...
	private static class Message {
		...
		public String toString() {
			return "<chat from=\"" + _from + "\" alert=\"" + _alert + "\">"
					+ encodeText(_text) + "</chat>";
		}
	...
	private class Member {
		...
		public void rename(Map room, String name) {
			...
			setName(name);
			if (oldName != null)
				ChatFilter.this.sendMessage(room, this, oldName
						+ " has been renamed to " + name, true);
		}
